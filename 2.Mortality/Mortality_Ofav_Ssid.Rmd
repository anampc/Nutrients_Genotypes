...
# Only treatment model
    ...
    # Plot the survival model
    Ss_Treatment_Only<-ggsurvplot(fit1, data = Survival.Ss.data, pval = TRUE, 
           conf.int = T, risk.table=T, palette=Fill.colour,
           break.time.by=15, xlim=c(0,115), risk.table.y.text = FALSE,
           risk.table.title="Number of fragments at risk") + ggtitle("(c)") 
    Ss_Treatment_Only
...
# Only treatment model
    ...
    # Plot the survival model
    Of_Treatment_Only<-ggsurvplot(fit5, data = Survival.Of.data, pval = TRUE, 
           conf.int = T, risk.table=T, palette=Fill.colour,
           break.time.by=15, xlim=c(0,115), risk.table.y.text = FALSE,
           risk.table.title="Number of fragments at risk") + ggtitle("(b)") 
    Of_Treatment_Only
...
# All Spp Treatment plot
```{r}
Survival.Trat.plot<-ggarrange(Ac_Treatment_Only$plot, Of_Treatment_Only$plot, Ss_Treatment_Only$plot, 
                              Ac_Treatment_Only$table, Of_Treatment_Only$table, Ss_Treatment_Only$table,
                              heights = c(2, 0.7),
                              ncol = 3, nrow = 2)
Survival.Trat.plot
