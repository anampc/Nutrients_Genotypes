...
# Import data: 
  ...
  ## Data type  
    ...
    BW.Control<-subset(BW.Tall, Treatment=="A")
...
# ggsave(file="Figure2_BW_corrected.svg", plot=BW_Treat, dpi = 300, units="in", width=3.5, height=3.0)
```
...
```{r, echo =FALSE}
BW_Colony<- ggplot(BW.Tall, aes (Days, dAW.gd, shape=factor(Treatment),
...
#BW_Colony + facet_grid(~MoteGen)
BW_Colony <- BW_Colony + facet_grid(~MoteGen)
BW_Colony
# ggsave(file="Figure2_BW_corrected_genotype.svg", plot=BW_Colony, dpi = 300, units="in", width=5, height=5)
...
BW_ColonyT<- ggplot(BW.Tall, aes (Days, dAW.gd, colour=factor(Fra), group=Fra)) +    stat_summary(fun.y=mean, geom="line") + ggthe_bw + facet_grid (MoteGen~Treatment)+  geom_point()+ theme(legend.position = "none")
BW_ColonyT
...
```{r, echo =FALSE}
BW_Init<- ggplot(BW.Initial, aes (MoteGen, dAW.gd)) +
...
Supplementary information per colony
...
#ggsave(file="S2a_Growth_genotype.svg", plot=BW_Col, width=6.0, height=3.5)
...
# Select model
  AIC(LM_1, LM_2, LM_3) # 1=2, but 3 has higher AIC
...
# Pair-wise comparisons
  ...
  write.csv(BW_groups, "BW_groups_corrected.csv", row.names = F)
