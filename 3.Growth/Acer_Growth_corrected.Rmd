...
# Import data: 
  ...
  ## Data type  
    ...
    BW.Control<-subset(BW.Tall, Treatment=="Ambient")
...
#BW_Colony + facet_grid(~MoteGen)
BW_Colony <- BW_Colony + facet_wrap(MoteGen~.)
BW_Colony
...
Data after nutrients (no baseline)

```{r}
BW <- ggplot(BW.Tall, aes(Days, dAW.gd, colour=factor(Treatment))) +
          stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2)+
          stat_summary(fun.y=mean, geom="point", size =3, alpha=0.5) +
