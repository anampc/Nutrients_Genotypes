...
# Import data: 
  ...
  ## Data type  
    ...
    #BW.Tall$TimePoint[BW.Tall$DayF=="-28"]<-"3"
    #BW.Tall$TimePoint[BW.Tall$DayF=="-7"]<-"4"
    #BW.Tall$TimePoint[BW.Tall$DayF=="28"]<-"9"
    #BW.Tall$TimePoint[BW.Tall$DayF=="62"]<-"11"
    #BW.Tall$TimePoint[BW.Tall$DayF=="75"]<-"13"
    #BW.Tall$TimePoint[BW.Tall$DayF=="91"]<-"16"
    #BW.Tall$TimePoint[BW.Tall$DayF=="100"]<-"17"
    #BW.Tall$TimePoint[BW.Tall$DayF=="113"]<-"22"
...
# Treatment effect over time
    BW_Treat<- ggplot(BW.Tall, aes (Days, dAW.gd, shape=factor(Treatment),
      ...
      ggthe_bw + Colour.colour + ggtitle("a")+
    ...
    theme(legend.position=c(0.2,0.3),
      ...
      annotate("segment", x = -30, xend = -2, y = -1.8, yend = -1.8,
                colour = "gray90")+
      annotate("text", x = -16, y = -1.5, label = "Baseline", size=3)+
    
      annotate("segment", x = 2, xend = 91, y = -1.8, yend = -1.8,
                colour = "gray90", linetype=2)+
      annotate("text", x = 45, y = -1.5, label = "Nutrients", size=3)+
    
      annotate("segment", x = 79, xend = 91, y = -1.8, yend = -1,
                colour = "gray90", linetype=3)+
    
      annotate("segment", x = 91, xend = 110, y = -1, yend = -1,
                colour = "gray90", linetype=3)+
      annotate("text", x = 99, y = -1.5, label = "Heat", size=3)
  BW_Treat
...
```{r}
Model_treatment<-lmer(dAW.gd~Treatment + (1|DayF) + (1|Genotype), data = BW.Tall)
summary(Model_treatment)
anova(Model_treatment)
ranova(Model_treatment)

Model_treatmentEM<-emmeans(Model_treatment,~Treatment)
 library(multcomp)
cld(Model_treatmentEM, "tukey")

# Pairwise comparisons
  BW.emmc<-emmeans(Model_treatmentEM, ~Treatment)
...
## Genotypes and nutrient treatment means over time

```{r, echo =FALSE}

BW_Col <- ggplot(BW.Tall, aes(Days, dAW.gd, colour=MoteGen)) + ggthe_bw +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 10,
                     position = position_dodge(4), alpha=0.8 )+
  stat_summary(fun.y=mean, geom="line", position = position_dodge(4),
...
## Genotypes and nutrients (pooled) means over time

```{r}
  BW.Tall$Treatment2 <- "N and N+P"
  BW.Tall$Treatment2[BW.Tall$Treatment == "A"] <- "A"
