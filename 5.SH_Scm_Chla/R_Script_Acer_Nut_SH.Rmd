...
```{r CalculateRatios}
  ...
  # Acer.Out <- steponeR(files=Acer.plates, target.ratios=c("A.Acer","B.Acer","C.Acer","D.Acer"), 
                      # fluor.norm=list(A=0, B=0, C=0, D=0, Acer=0),
                      # copy.number=list(A=1, B=1, C=1, D=1, Acer=1),
                      # ploidy=list(A=1, Acer=2),
                      # extract=list(A=0.813, B=0.813, C=0.813, D=0.813, Acer=0.982))
...
# Correct blastate samples for extraction efficeincy of 95.5%
...
# 3.If coral detected in one technical replicate, remove the sample
  ...
  HigtSH<-Acer[which(Acer$A.Acer>0.9), ]
  Acer <- droplevels(Acer[!rownames(Acer) %in% rownames(HigtSH), ])
  ...
  ToRe_Run1<-rbind(ToRe_Run1, HigtSH) 
  ToRe_Run1<-anti_join (ToRe_Run1, Acer, by=c("Sample"))
  
```
    

### 6. Quality control: Check CTs in plates, smpling points, etc...
...
skim(Acer)
capture.output(qPCRsummary, file = "qPCRsummary.txt")

# Comparission bw MM and SYBR
