...
# Ofav    
    YII.Ofav<-read.csv("YII_Data/YII_Nutrients_Ofav.csv")
      ...
      YII.Ofav$Genotype<-paste("Ofav", YII.Ofav$Colony, sep = "_")
...
# Ssid      
    YII.Ssid<-read.csv("YII_Data/YII_Nutrients_Ssid.csv")
      ...
      YII.Ssid$Genotype<-paste("Ssid", YII.Ssid$Colony, sep = "_")
      
      Ssid.cores<-YII.Ssid %>% group_by(Fragment, Colony, Treatment) %>% count(Replicate)
      Ssid.cores$Fragment<-as.character(Ssid.cores$Fragment)
      write.csv(Ssid.cores, "Ssid_Fragments.csv", row.names = F)
...
# Organize data type
      ...
      YII.data$Genotype<-factor(as.character(YII.data$Genotype), 
                             levels=c("Acer_07","Acer_08","Acer_31",
                                      "Acer_48","Acer_50","Acer_62", 
                             "Ofav_31","Ofav_6","Ofav_20","Ofav_34", 
                             "Ssid_30","Ssid_20","Ssid_24",
                             "Ssid_28","Ssid_22","Ssid_23","Ssid_27"))  # D dominance order
...
YII_Treat_BW<- ggplot(YII.data, aes (Days, YII, colour=factor(Treatment), shape=factor(Treatment))) +
      ...
      Figure3<-YII_Treat_BW +facet_grid (Spp~.)+
        ...
        annotate("text", x = 99, y = 0.2, label = "Bleaching", size=3)
Figure3
