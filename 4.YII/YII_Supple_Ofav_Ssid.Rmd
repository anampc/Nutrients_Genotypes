...
```{r}
 # Experiment time points (metadata) 
    TimePoints<-read.csv("YII_Data/Time_Points.csv")
 ...
 #Ofav    
    YII.Ofav<-read.csv("YII_Data/YII_Nutrients_Ofav.csv")
      ...
      YII.Ofav$Colony<-factor(as.character(YII.Ofav$Colony), 
                            levels=c("34","20", "6", "31"))  # D dominance order
      YII.Ofav$Genotype<-paste("Of", YII.Ofav$Colony, sep = "_")
      YII.Ofav$Spp<-"Of"
      YII.Ofav$Initial_Sym<-NULL
      
      YII.Ofav$Initial_Sym[YII.Ofav$Genotype=="Of_31"]<-"B"
      YII.Ofav$Initial_Sym[YII.Ofav$Genotype!="Of_31"]<-"D"

            
# Ssid      
    YII.Ssid<-read.csv("YII_Data/YII_Nutrients_Ssid.csv")
      YII.Ssid$Colony<-factor(as.character(YII.Ssid$Colony), 
                                 levels=c("22","23","27", "28",
                                          "20", "24","30") )  # D dominance order
      YII.Ssid$Genotype<-paste("Ss", YII.Ssid$Colony, sep = "_")
      YII.Ssid$Spp<-"Ss"
      
      YII.Ssid$Initial_Sym[YII.Ssid$Genotype=="Ss_22"|
                           YII.Ssid$Genotype=="Ss_23"|
                           YII.Ssid$Genotype=="Ss_27"]<-"D"
      
      YII.Ssid$Initial_Sym[YII.Ssid$Genotype=="Ss_20"|
                           YII.Ssid$Genotype=="Ss_28"]<-"C1"
      
      YII.Ssid$Initial_Sym[YII.Ssid$Genotype=="Ss_24"|
                           YII.Ssid$Genotype=="Ss_30"]<-"C3"

  # Pool all species the data
      YII.data<-dplyr::bind_rows(list(YII.Ofav,YII.Ssid))
      YII.data<-YII.data[,-1]
...
# Organize data type
      ...
      YII.data$Treatment <- as.character(YII.data$Treatment)
   ...
   # Add timepoints
      YII.data<-left_join(YII.data, TimePoints, by="Date")
      YII.data$SampleID <- paste(YII.data$Fragment, YII.data$Time_Point, sep = "_")
...
YII_Treat_OfSs<- ggplot(data=subset(YII.data, Spp!="Ac"), aes (Days, YII, colour=factor(Treatment), shape=factor(Treatment))) + ggtitle("b") +
    ...
    FigureS4+facet_wrap(Initial_Sym~Spp)
...
FigureS4b<-YII_Treat_Spp_Sym + facet_wrap (Initial_Sym~Spp)+
...
```{r}
YII_Treat_Sym_Spp<- ggplot(data=subset(YII.data), aes (Days, YII, colour=factor(Initial_Sym))) +
...
YII.Ofav.H<-subset(YII.Ofav, Days>76)
...
YII.Ssid.H<-subset(YII.Ssid, Days>76)
...
LME_Ofav0<-lmer(YII ~ Treatment + Initial_Sym + (1|Replicate), 
                     data=YII.Ofav.0)
     #step (LME_Ofav0) 
...
LME_Ofav0<-lm(YII ~ Initial_Sym, data=YII.Ofav.0)
     step (LME_Ofav0) 
     anova(LME_Ofav0)         

# EMMs
    Ofav.YII.emm0<-emmeans(LME_Ofav0, ~ Initial_Sym)
      # contrast(Ssid.YII.emm, "tukey")
      
      # Tukey comparison (do not trust CI to compare EMMs)
      plot(emmeans(LME_Ofav0, ~Initial_Sym), comparisons = TRUE) +
...
LME_Ssid0<-lmer(YII ~ Treatment + Initial_Sym +(1|Replicate), 
                     data=YII.Ssid.0)
...
# EMMs
      Ssid.YII.emm0<-emmeans(LME_Ssid0, ~ Initial_Sym)
      # contrast(Ssid.YII.emm, "tukey")
      
      # Tukey comparison (do not trust CI to compare EMMs)
      plot(emmeans(LME_Ssid0, ~Initial_Sym), comparisons = TRUE) +
        coord_flip(xlim = NULL, ylim = NULL, expand = TRUE) +
        theme_bw()
...
```{r}
LME_Ofav1.1<-lmer(YII ~ Treatment * Initial_Sym + (1|Fragment), 
                     data=YII.Ofav.C.1)
...
# EMMs
    Ofav.YII.emm1.1<-emmeans(LME_Ofav1.1, ~ Initial_Sym|Treatment)
    # contrast(Ssid.YII.emm, "tukey")
      
    # Tukey comparison (do not trust CI to compare EMMs)
      plot(emmeans(LME_Ofav1.1, ~Treatment|Initial_Sym), comparisons = TRUE) +
      coord_flip(xlim = NULL, ylim = NULL, expand = TRUE) + facet_grid(~Initial_Sym) +
        theme_bw()
...
# 1. Ofav- Days 
  ...
  Model_of_1b_plot + facet_grid(~Initial_Sym)
...
```{r}
# 1. Ofav- Days 
      LME_Ofav<-lmer(YII ~ Treatment * DaysF * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
                        data=YII.Ofav.C, na.action=na.omit)
      lmerTest::step (LME_Ofav) # Replicate is not significant
      ranova(LME_Ofav)
      
      LME_Ofav1.2<-lmer(YII ~ Treatment * DaysF * Initial_Sym + 
                        (1|Fragment),
                      data=YII.Ofav.C, na.action=na.omit)
  ...
  Model_of_1.2_plot + facet_grid(~Initial_Sym)
  
      # EMMs
      Ofav.YII.emmC<-emmeans(LME_Ofav1.2, ~Treatment * DaysF * Initial_Sym)
...
```{r}
LME_Ssid1.1<-lmer(YII ~ Treatment * Initial_Sym + (1|Fragment), 
                     data=YII.Ssid.C.1)
...
# EMMs
    Ssid.YII.emm1.1<-emmeans(LME_Ssid1.1, ~ Initial_Sym|Treatment)
    # contrast(Ssid.YII.emm, "tukey")
      
    # Tukey comparison (do not trust CI to compare EMMs)
      plot(emmeans(LME_Ssid1.1, ~Treatment|Initial_Sym), comparisons = TRUE) +
      coord_flip(xlim = NULL, ylim = NULL, expand = TRUE) + facet_grid(~Initial_Sym)+
        theme_bw()
      
      Ssid.YII.Groups1.1<-cld(Ssid.YII.emm1.1, by=NULL) # compact-letter display
      Ssid.YII.Groups1.1<- Ssid.YII.Groups1.1[order(
        Ssid.YII.Groups1.1$Initial_Sym, 
...
```{r}
LME_Ssid<-lmer(YII ~ Treatment * Days * Initial_Sym +
...
LME_Ssid1<-lmer(YII ~ Treatment * Days * Initial_Sym +
                       (1|Fragment),
                     data=YII.Ssid.C, na.action=na.omit)


# EMMs
      Ssid.YII.emm1<-emmeans(LME_Ssid1, ~Treatment * Days * Initial_Sym)
      # contrast(Ssid.YII.emm, "tukey")
      
      # Effect plot options
      emmip(LME_Ssid1, ~Initial_Sym|Treatment, CIs = TRUE) + theme_bw() # interaction plot of predictions
...
# 4. Plot
  ...
  Model1Ss_plot + facet_grid(~Initial_Sym)
```
...
```{r}
# 1. Ssid- Days 
      LME_Ssid<-lmer(YII ~ Treatment * DaysF * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
                        data=YII.Ssid.C, na.action=na.omit)
      lmerTest::step (LME_Ssid) # Replicate is not significant
      ranova(LME_Ssid)
      
      LME_Ssid1.2<-lmer(YII ~ Treatment * DaysF * Initial_Sym + 
                        (1|Fragment),
                      data=YII.Ssid.C, na.action=na.omit)
...
# 4. Plot
  ...
  Model_Ss_1.2_plot + facet_grid(~Initial_Sym)
```

```{r}
# 5. EMMs
    Ssid.YII.emmC<-emmeans(LME_Ssid1.2, ~Treatment * DaysF * Initial_Sym)
      #contrast(Ssid.YII.emm, "tukey")
...
```{r}
# 1. Ofav- Days 
      LME_Ofav.H<-lmer(YII ~ Treatment * Days * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
...
```{r}
 # 4 .Plot
  ...
  Model_of_H_plot
  Model_of_H_plot+ facet_grid(Initial_Sym~.)
...
```{r}
# 1. Ofav- Days 
      LME_Ofav.HF<-lmer(YII ~ Treatment * DaysF * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
                        data=YII.Ofav.H, na.action=na.omit)
      lmerTest::step (LME_Ofav.HF) # Replicate is not significant
      ranova(LME_Ofav.HF)
      
      LME_Ofav.HF.1<-lmer(YII ~ Treatment + DaysF + Initial_Sym + (1 | Fragment) +
                      Treatment:DaysF + DaysF:Initial_Sym,
                      data=YII.Ofav.H, na.action=na.omit)
...
# 4. Plot
  ...
  Model_of_HF_plot
  Model_of_HF_plot + facet_grid(Initial_Sym~.)
```

```{r}
# 5. EMMs
      Ofav.YII.emmH<-emmeans(LME_Ofav.HF.1, ~ Treatment:DaysF + DaysF:Initial_Sym)
      ...
      Ofav.YII_groupsH<-Ofav.YII_groupsH[order(
        ...
        Ofav.YII_groupsH$Initial_Sym),]
      Ofav.YII_groupsH
      #write.csv(Ofav.YII_groupsH, "Outputs/Multicomp_OfavYIIH.csv", row.names = F)
```
...
```{r}
# 1. Ssid- Days 
      LME_Ssid.H<-lmer(YII ~ Treatment * Days * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
                        data=YII.Ssid.H, na.action=na.omit)
      lmerTest::step (LME_Ssid.H) # Replicate is not significant
      ranova(LME_Ssid.H)
      
      LME_Ssid.H1<-lmer(YII ~ Treatment * Days * Initial_Sym + 
                        (1 | Genotype/Fragment),
                      data=YII.Ssid.H, na.action=na.omit)
...
```{r, fig.height= 6, fig.width = 8}
 # 4 .Plot
  ...
  Model_Ss_H_plot+ facet_grid(~Initial_Sym)
```
...
```{r}
# 1. Ssid- Days 
      LME_Ssid.HF<-lmer(YII ~ Treatment * DaysF * Initial_Sym +
                       (1|Genotype) + (1|Fragment) +(1|Replicate), 
                        data=YII.Ssid.H, na.action=na.omit)
      lmerTest::step (LME_Ssid.HF) # Replicate is not significant
      ranova(LME_Ssid.HF)
      
      LME_Ssid.HF.1<-lmer(YII ~ Treatment * DaysF * Initial_Sym + (1 | Fragment),
                      data=YII.Ssid.H, na.action=na.omit)
...
# 4. Plot
  ...
  Model_Ss_HF_plot + facet_grid(~Initial_Sym)
```

```{r}
# 5. EMMs
    Ssid.YII.emmH<-emmeans(LME_Ssid.HF.1, ~ Treatment*DaysF*Initial_Sym)
