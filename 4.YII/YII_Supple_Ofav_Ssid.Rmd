...
YII_Treat_Spp_Sym<- ggplot(data=subset(YII.data), aes (Days, YII, colour=factor(Treatment))) +
         ggthe_bw + Fill.colour +
  #geom_jitter(shape=21, size=0.05)+
  ...
  annotate("segment", x = 79, xend = 91, y = 0.13, yend = 0.2,
                  colour = "gray", linetype=3)+
  annotate("segment", x = 91, xend = 110, y = 0.2, yend = 0.2,
                  colour = "gray", linetype=3)

        
FigureS4b<-YII_Treat_Spp_Sym + facet_wrap (Spp~InitialCommunity)+
        annotate("text", x = 45, y = 0.15, label = "Nutrients", size=3)+
        annotate("text", x = 99, y = 0.15, label = "Heat", size=3)
FigureS4b

#ggsave(file="Outputs/Fig_3_YII_Treat.svg", plot=FigureS4b, width=7, height=6)
```
