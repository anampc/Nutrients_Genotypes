...
D.PTreatment <- ggplot(qPCR.variables, aes(Days, D.Prp, colour=Fragment)) +
        ggthe_bw +  theme(legend.position="none") +  
        #geom_jitter(aes(colour=factor(Replicate))) +
        geom_line() + facet_grid (Treatment~Genotype) +
      ...
      scale_x_continuous(name="Days in the experiment",
                         ...
                         expand = c(0, 0))
D.PTreatment
#ggsave(file="Outputs/D.P_Treatment_Genotype.svg", plot=D.PTreatment, width=4.0, height=4)
...
```{r}
LME_BaseLine<-lmer(logSH ~ Treatment + Genotype + (1|Replicate), 
                     data=SH.0)
   #step(LME_BaseLine)
   anova(LME_BaseLine)
   ranova(LME_BaseLine)
...
```{r}
# Multicomp    
    Ofav.YII.emm<-emmeans(LM_Ofav.qpCR, ~Treatment * DaysF)
