...
```{r cars}
# Read data
  data<-read.csv("Data/All_Blast.csv")
  summary (data)
  
# Organize factors  
  data$Genotype <- as.factor(paste(data$Spp, data$Colony, sep = "_"))
  data$Fragment <- as.factor(paste(data$Spp, data$Fragment, sep = "_"))
...
data <- data[!(data$Genotype == "A.cer_F"), ]
...
Merge blast data with S/H cell ratio

```{r}
qPCR<-read.csv("Data/All_SPP_SH_cell_ratio_Community.csv", header = T)
data<-left_join(data, qPCR, by = "Previous.sample")
  
  data$Community[which(data$Genotype=="S.sid_30")]<-"C3"
  
  data$Community[which(data$Genotype=="S.sid_22")]<-"D"
  data$Community[which(data$Genotype=="S.sid_23")]<-"D"
  data$Community[which(data$Genotype=="S.sid_27")]<-"D"
  #data$Community[which(data$Previous.sample=="Ss_27-97_T13")]<-"D"
  #data$Community[which(data$Previous.sample=="Ss_22-3_T22")]<-"D"
  #data$Community[which(data$Previous.sample=="Ss_27-87_T22")]<-"D"
  #data$Community[which(data$Previous.sample=="Ss_22-21_T13")]<-"D"
  
  data$Community[which(data$Genotype=="S.sid_20")]<-"C1"
  #data$Community[which(data$Previous.sample=="Ss_20-1_T13")]<-"C1"
  
  
  data$Community[which(data$Previous.sample=="Ss_24-23_T13")]<-"C3"
  data$Community[which(data$Previous.sample=="Ss_24-6_T22")]<-"C3"
  data$Community[which(data$Previous.sample=="Ss_24-25_T22")]<-"C3"
  data$Community[which(data$Previous.sample=="Ss_28-3_T22")]<-"D"
  data$Community[which(data$Previous.sample=="Ss_28-14_T22")]<-"C1"
...
```{r}
MOTE<-read.csv("Data/Genotypes_mote.csv", header = TRUE)
Acer.data$Genotype<-NULL

Acer.data<-merge(Acer.data, MOTE, 
  ...
  Acer.data$Survivorship[Acer.data$Genotype=="G_08"|
                      ...
                      Acer.data$Genotype=="G_50"]<-"Suceptible"
summary(Acer.data)    
...
* Symbiodiniaceae/ cm2 
```{r}
Acer_Colony<- ggplot(Acer.data, aes (Days, Sym.cm2, colour=Genotype)) +
  ggthe_bw +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", 
               position = position_dodge(width = 1), width = 0.2 )+
  stat_summary(fun.y=mean, geom="line") +   theme_gdocs()+
  geom_point(alpha=0.5) + facet_grid (Spp~Treatment, scales="free")
Acer_Colony + facet_grid(Survivorship~Treatment)
...
AcerChl_Treatment<- ggplot(Acer.data, aes (Days, Chl_a, colour=Treatment,
  ...
  ggtitle("(c)")
AcerChl_Treatment
AcerChl_Treatment + facet_wrap(~Survivorship)
```
...
Sy.Summary <- plyr::ddply (Ofav_Ssid.data, . (Spp, Treatment, Days),
              summarise,
              Sy_mean = mean (Sym.cm2, na.rm = T),
              Sy_sd = sd (Sym.cm2, na.rm = T))
Sy.Summary
...
Chl.Summary <- plyr::ddply (Ofav_Ssid.data, . (Spp, Treatment, Days),
              summarise,
              Chl_mean = mean (Chl_a, na.rm = T),
              Chl_sd = sd (Chl_a, na.rm = T))
Chl.Summary
...
Ofav.data<-droplevels(Ofav.data)
  ...
  Ofav_Sy.Summary <- plyr::ddply (Ofav.data, . (Treatment, Days),
              summarise,
              Sy_mean = mean (Sym.cm2, na.rm = T),
              Sy_sd = sd (Sym.cm2, na.rm = T))
  Ofav_Sy.Summary
    
  Ofav_Ch.Summary <- plyr::ddply (Ofav.data, . (Treatment, Days),
              summarise,
              Sy_mean = mean (Chl_a, na.rm = T),
              Sy_sd = sd (Chl_a, na.rm = T))
  Ofav_Ch.Summary
...
```{r}
# Sym
LMER_Sym_Ofav<-lmer(Zoox.sq ~ Treatment * DaysF + 
...
```{r}
# Multicomp
    Ofav.Sym.emm<-emmeans(LMER_Sym_Ofav3, ~Treatment * DaysF)
...
Ssid_Sy.Summary <- plyr::ddply (Ssid.data, . (Treatment, Days),
              summarise,
              Sy_mean = mean (Sym.cm2, na.rm = T),
              Sy_sd = sd (Sym.cm2, na.rm = T))
  Ssid_Sy.Summary
    
  Ssid_Ch.Summary <- plyr::ddply (Ssid.data, . (Treatment, Days),
              summarise,
              Sy_mean = mean (Chl_a, na.rm = T),
              Sy_sd = sd (Chl_a, na.rm = T))
  Ssid_Ch.Summary
...
```{r}
# Chl-a.cm2 
  shapiro.test(Ssid.data$Chl_a) # Not ormal
...
```{r}
# Sym
LMER_Sym_Ssid<-lmer(Sym.cm2 ~ Treatment * DaysF * Community + 
                 (1|Rep) + (1|Colony), data=Ssid.data, 
                 na.action = na.omit)
    anova(LMER_Sym_Ssid)
...
```{r}
# Multicomp
    Ssid.Sym.emm<-emmeans(LMER_Sym_Ssid3, ~DaysF * Community)
...
LMER_Chla_Ssid2<-lmer(Chl_a ~ Treatment + DaysF + Community + (1 | Colony) + 
...
# Multicomp
  Ssid.Chla.emm<-emmeans(LMER_Chla_Ssid2, ~Treatment * DaysF)
    ...
    Ssid.Chla.Groups
  #write.csv(Ssid.Chla.Groups, "Outputs/Multicomp_SsidChla.csv", row.names = F)
